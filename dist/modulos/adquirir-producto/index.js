...
(function (AdquirirProducto) {
    ...
    (function (Metodos) {
        Metodos.sendMessage = function (msg) {
            Data.Chats.actualizarChat(msg.chat.id, index_1.Contextos.AdquirirProducto.Index.index, constants_1.Comandos.AdquirirProducto.Index.verProductosDisponibles).then(function () {
                var messageOptions = {
                    reply_markup: {
                        inline_keyboard: [
                            [
                ...
                initBot_1.bot.sendMessage(msg.chat.id, "Selecciona uno de los productos disponibles", messageOptions);
            });
        };
            ...
            initBot_1.bot.on('chosen_inline_result', function (msg) {
                Data.Chats.getChatByUserId(msg.from.id).then(function (chat) {
                    if (chat.contexto.indexOf(index_1.Contextos.AdquirirProducto.Index.index) === 0 &&
                        chat.comando.indexOf(constants_1.Comandos.AdquirirProducto.Index.verProductosDisponibles) === 0) {
                        if (msg.result_id == -1) {
                            Data.Chats.actualizarChat(msg.from.id, index_1.Contextos.PaginaInicial.menuPrincipal, '');
                            return;
                        }
